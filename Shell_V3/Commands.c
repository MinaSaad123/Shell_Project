...
void mytype(char *Arg[], int ArgNum)
{
    ...
    if (ArgNum == 0)
	{
		if (write(STDOUT, "You didn't pass argument\n", strlen("You didn't pass argument\n")) == -1)
       	{
       		perror("mytype");
       	}
	...
	if (flag == 1)
	{
		if ( write(STDOUT, "This command is built-in\n", strlen("This command is built-in\n")) == -1 )
		{
			perror("mytype");
			return;
		}
	...
	} else
	{
		...
		while (c >= 0)
		{
			if ( ( OpenedDir = opendir(env[c]) ) == NULL )
			{
				perror("mytype");
				return;
			}
			...
			{
				if ( strcmp(Entry->d_name, Arg[0]) == 0 )
				{
					if (write(STDOUT, "This command is External command\n", strlen("This command is External command\n")) == -1)
   					{
        				perror("mytype");
        				return;
    				}
		...
		if ( flag == 0 )
		{
			if (write(STDOUT, "This command is not exist\n", strlen("This command is not exist\n")) == -1)
   			{
        		perror("mytype");
        		return;
    		}
...
void myfree(int ArgNum, int OptnNum)
{
	...
	printf("Mem   total:%luMB   used:%luMB   free:%luMB\n", (info.totalram * info.mem_unit) / 1024, (info.totalram * info.mem_unit - info.freeram * info.mem_unit) / 1024, (info.freeram * info.mem_unit) / 1024 );
